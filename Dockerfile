FROM avhost/docker-matrix:v1.2.1
...
RUN apt-get update -y -q --fix-missing \
    ...
    && wget -O /usr/local/lib/python3.7/dist-packages/synapse/rest_auth_provider.py  https://raw.githubusercontent.com/kamax-matrix/matrix-synapse-rest-auth/master/rest_auth_provider.py \
    && apt-get clean \ 
    && pip3 install bleach \
